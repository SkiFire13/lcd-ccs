...
inv-conv-trans = helper refl refl
  ...
  helper {par pl pr} {tau} refl refl (par-B {send _} rl rr) with inv-conv-trans {pl} rl | inv-conv-trans {pr} rr
  ... | pl' , refl , rl' | pr' , refl , rr' = par pl' pr' , refl , par-B rl' rr'
  helper {par pl pr} {tau} refl refl (par-B {recv _} rl rr) with inv-conv-trans {pl} rl | inv-conv-trans {pr} rr
  ... | pl' , refl , rl' | pr' , refl , rr' = par pl' pr' , refl , par-B rl' rr'
  helper {par pl pr} {tau} refl refl (par-B {tau} rl rr) with inv-conv-trans {pl} rl | inv-conv-trans {pr} rr
  ... | pl' , refl , rl' | pr' , refl , rr' = par pl' pr' , refl , par-B rl' rr'
  
  helper {indet f} {a} refl refl (indet {s = s} r) with inv-conv-trans {f s} {a} r
  ... | p' , refl , r' = p' , refl , indet {s = s} r'
  
  helper {const n args} refl refl (const r) with inv-conv-trans {penv n args} r
  ... | p' , refl , r' = p' , refl , const r'
  
  helper {rename f p} refl e (rename r) with inv-conv-map-rename-eq e
  ... | _ , refl , refl with inv-conv-trans {p} r
  ... | p' , refl , r' = rename f p' , refl , rename r'
  
  helper {hide f p} {a} refl refl (hide {z = z} r) with inv-conv-trans {p} r
  ... | p' , refl , r' = hide f p' , refl , hide {z = inv-conv-hide-z {f} {a} z} r'

  helper {if false _} refl refl (indet {s = ()} r)
  helper {if true p} refl refl r with inv-conv-trans {p} r
