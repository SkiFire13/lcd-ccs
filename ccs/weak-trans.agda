...
w-hide : ∀ {f} → {z : T (filter-act f a)} → WeakTrans p a q → WeakTrans (hide f p) a (hide f q)
...
w-rename : ∀ {f} → WeakTrans p a q → WeakTrans (rename f p) (map-act f a) (rename f q)
